...
class TestRestApiIntegration extends TestCase {
	...
	public function test_get_licenses_endpoint() {
		...
		$license1_id = $this->create_test_license( array( 'name' => 'API Test License 1' ) );
		$license2_id = $this->create_test_license( array( 'name' => 'API Test License 2' ) );
	...
	public function test_olp_introspect_valid_token() {
		// Create OAuth client
		$client_data = $this->create_test_oauth_client( array( 'client_name' => 'Introspect API Test' ) );
		...
		$request->set_body( json_encode( array( 'token' => $token ) ) );
	...
	public function test_api_response_format_consistency() {
		...
		$expected_fields = array( 'id', 'name', 'description', 'content_url', 'payment_type', 'amount', 'currency', 'xml_url' );
	...
	public function test_api_error_handling_consistency() {
		$endpoints_and_errors = array(
			array( '/rsl/v1/licenses/99999', 'GET', 404, 'license_not_found' ),
			array( '/rsl/v1/validate', 'POST', 400, 'rest_missing_callback_param' ), // Missing content_url
			array( '/rsl-olp/v1/token', 'POST', 400, 'invalid_request' ), // Missing parameters
		);
		...
		foreach ( $endpoints_and_errors as [$endpoint, $method, $expected_status, $expected_code] ) {
			...
			if ( $method === 'POST' ) {
				$request->set_header( 'Content-Type', 'application/json' );
				$request->set_body( json_encode( array() ) ); // Empty body to trigger validation errors
			}
	...
	public function test_api_pagination() {
		...
		for ( $i = 1; $i <= 15; $i++ ) {
			$this->create_test_license( array( 'name' => "Pagination Test License {$i}" ) );
		}
