...
class TestEndToEndWorkflows extends TestCase {
    ...
    public function test_complete_license_lifecycle_workflow() {
        ...
        $update_result = $this->license_handler->update_license($license_id, [
        ...
        $deactivate_result = $this->license_handler->update_license($license_id, ['active' => 0]);
        ...
        $reactivate_result = $this->license_handler->update_license($license_id, ['active' => 1]);
    ...
    public function test_complete_frontend_integration_workflow() {
        ...
        $shortcode_output = $this->frontend->license_shortcode(['format' => 'link', 'text' => 'Our License']);
    ...
    public function test_complete_multi_license_workflow() {
        ...
        $test_cases = [
            ['http://example.org/', $root_license_id, 'Root License'],
            ['http://example.org/api/v1/data', $api_license_id, 'API License'],
            ['http://example.org/premium/', $premium_license_id, 'Premium Content License'],
            ['http://example.org/blog/post', $root_license_id, 'Root License'], // Falls back to root
        ];
