...
class TestBasicFunctionality extends TestCase {
	...
	public function test_wordpress_function_mocks() {
		// Test wp_parse_args
		$result = wp_parse_args( array( 'test' => 'value' ), array( 'default' => 'default_value' ) );
	...
	public function test_wp_error_class() {
		$error = new \WP_Error( 'test_error', 'Test error message', array( 'test_data' => 'test_value' ) );

		$this->assertEquals( 'test_error', $error->get_error_code() );
		$this->assertEquals( 'Test error message', $error->get_error_message() );
		$this->assertEquals( array( 'test_data' => 'test_value' ), $error->get_error_data() );
	}
	...
	public function test_testcase_helpers() {
		...
		$license_id = $this->create_test_license( array( 'name' => 'Helper Test License' ) );
		$this->assertIsInt( $license_id );
		$this->assertGreaterThan( 0, $license_id );

		// Test create_test_oauth_client
		$client_data = $this->create_test_oauth_client( array( 'client_name' => 'Helper Test Client' ) );
		...
		$token = $this->generate_test_jwt( array( 'sub' => 'test-subject' ) );
