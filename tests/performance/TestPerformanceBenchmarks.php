...
class TestPerformanceBenchmarks extends TestCase {
    ...
    private function warm_up_system() {
        ...
        $warmup_license_id = $this->create_test_license(['name' => 'Warmup License']);
    ...
    public function test_license_retrieval_performance() {
        ...
        $license_ids = [];
        for ($i = 0; $i < 50; $i++) {
            $license_ids[] = $this->create_test_license(['name' => "Retrieval Test {$i}"]);
        }
        ...
        for ($i = 0; $i < $iterations; $i++) {
            ...
            $this->assertLessThan(self::DATABASE_QUERY_THRESHOLD, $execution_time, 
    ...
    public function test_xml_generation_performance() {
        ...
        for ($i = 0; $i < $iterations; $i++) {
            ...
            $this->assertLessThan(self::XML_GENERATION_THRESHOLD, $execution_time, 
    ...
    public function test_jwt_token_generation_performance() {
        ...
        for ($i = 0; $i < $iterations; $i++) {
            ...
            $this->assertLessThan(self::TOKEN_GENERATION_THRESHOLD, $execution_time, 
    ...
    public function test_jwt_token_validation_performance() {
        ...
        for ($i = 0; $i < $iterations; $i++) {
            ...
            $this->assertLessThan(self::LICENSE_VALIDATION_THRESHOLD, $execution_time, 
    ...
    public function test_oauth_client_validation_performance() {
        ...
        for ($i = 0; $i < $iterations; $i++) {
            ...
            $this->assertLessThan(100, $execution_time, 
    ...
    public function test_api_endpoint_performance() {
        ...
        $endpoints = [
            ['GET', '/rsl/v1/licenses'],
            ['GET', "/rsl/v1/licenses/{$license_id}"],
            ['POST', '/rsl/v1/validate', ['content_url' => 'http://example.org/test']],
        ];
        ...
        foreach ($endpoints as $endpoint_config) {
            ...
            for ($i = 0; $i < $iterations; $i++) {
                ...
                $this->assertLessThan(self::API_RESPONSE_THRESHOLD, $execution_time, 
                    "API endpoint {$method} {$route} took too long: {$execution_time}ms");
            }

            $average_time = $total_time / $iterations;
            $this->assertLessThan(100, $average_time, 
    ...
    public function test_memory_usage_optimization() {
        ...
        $this->assertLessThan($max_acceptable_memory, $memory_increase, 
    ...
    public function test_database_query_optimization() {
        ...
        $query_tests = [
            ...
            'get_active_licenses' => function() {
                return $this->license_handler->get_licenses(['active' => 1]);
            },
        ...
        foreach ($benchmarks as $operation => $threshold) {
            ...
            switch ($operation) {
                case 'license_creation':
                    $this->create_test_license(['name' => 'Regression Test License']);
                    break;
                    
                case 'license_retrieval':
                    $license_id = $this->create_test_license(['name' => 'Retrieval Test']);
                    $this->license_handler->get_license($license_id);
                    break;
                    
                case 'xml_generation':
                    $license_data = ['id' => 1, 'name' => 'XML Test', 'content_url' => '/test'];
                    $this->license_handler->generate_rsl_xml($license_data);
                    break;
                    
                case 'jwt_generation':
                    $payload = ['sub' => 'test', 'lic' => 1, 'exp' => time() + 3600];
                    $reflection = new \ReflectionClass($this->server);
                    $method = $reflection->getMethod('jwt_encode_payload');
                    $method->setAccessible(true);
                ...
                case 'jwt_validation':
                    $token = $this->generate_test_jwt(['sub' => 'test']);
                    $reflection = new \ReflectionClass($this->server);
                    $method = $reflection->getMethod('jwt_decode_token');
                    $method->setAccessible(true);
            ...
            $this->assertLessThan($threshold, $execution_time, 
